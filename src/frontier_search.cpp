...
using costmap_2d::FREE_SPACE;   /* static const unsigned char NO_INFORMATION = 255;static const unsigned char LETHAL_OBSTACLE = 254;static const unsigned char INSCRIBED_INFLATED_OBSTACLE = 253;static const unsigned char FREE_SPACE = 0; */
...
std::vector<Frontier> FrontierSearch::searchFrom(geometry_msgs::Point position  /* 传入的是 global_pose.position */)
{
  ...
  std::vector<bool> frontier_flag(size_x_ * size_y_, false);   /* 访问过 的cell*/
  std::vector<bool> visited_flag(size_x_ * size_y_, false);   /* 边沿 cell */

  // initialize breadth first search
  std::queue<unsigned int> bfs;     /* 手写的  宽度搜索？ */
...
Frontier FrontierSearch::buildNewFrontier(unsigned int initial_cell,  /* 宽度搜索时，正在处理的cell */
                                          unsigned int reference, /* 机器人所在的位置 */
                                       ...
                                       const std::vector<bool>& frontier_flag)
{
  ...
  if (map_[idx] != NO_INFORMATION || frontier_flag[idx]) {   /* NO_INFORMATION和障碍物的value，都比free cell的值要大。所以这里要确定边界是 NO_INFORMATION，而非障碍物。  */
    return false;
  }
